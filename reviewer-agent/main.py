...
class ReviewerAgent:
    ...
    def _try_review_pr(self, pr: PullRequest) -> bool:
        ...
        try:
            ...
            # Save policy candidates generated by the review
            self._save_policy_candidates(
                review_data.get("policy_candidates", []),
    ...
    def _save_policy_candidates(
        self,
        candidates: list[dict],
        linked_issue: Issue | None,
    ) -> None:
        """
        Save policy candidates from a review to PolicyStore and post a GitHub comment.

        Skipped silently when:
        - candidates list is empty
        - config.policy_db is not configured
        """
        if not candidates:
            return
        ...
        policy_store = PolicyStore(self.config.policy_db)
        try:
            candidate_ids: list[str] = []
            source_task = str(linked_issue.number) if linked_issue else None
            for candidate in candidates:
                cid = policy_store.insert_candidate(candidate, source_task=source_task)
                candidate_ids.append(cid)
                logger.info(
                    f"Policy candidate saved: {cid} — {candidate.get('title', '')}"
                )

            if linked_issue and candidate_ids:
                comment = _format_policy_candidate_comment(candidates, candidate_ids)
                self.github.comment_issue(linked_issue.number, comment)
                logger.info(
                    f"Posted {len(candidate_ids)} policy candidate(s) to issue #{linked_issue.number}"
                )
        finally:
            policy_store.close()
...
def _format_policy_candidate_comment(candidates: list[dict], ids: list[str]) -> str:
    """Format a GitHub Issue comment for policy candidate approval requests."""
    lines = [
        "## 🔍 Policy Candidates from Review",
        "",
        "以下のpolicy candidatesが検出されました。承認する場合は `approve-policy` ラベルを付けてください。",
        "",
    ]
    for candidate, cid in zip(candidates, ids):
        lines.append(f"### `{cid}`: {candidate.get('title', '(no title)')}")
        lines.append(f"**Why:** {candidate.get('why', '')}")
        lines.append(f"**Strength:** {candidate.get('strength', 'medium')}")
        lines.append("**Rules:**")
